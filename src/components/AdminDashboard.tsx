...
function ManualUserForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (draft.accessMode === "temporary-password" && !isStrongPassword(draft.temporaryPassword)) {
      setStatus("error");
  ...
  const resultMessage = errorCode === "invalid" ? t("admin.manualUserForm.validation") : errorCode === "weakPassword" ? t("passwordWeak") : errorCode === "endpointMissing" ? t("admin.manualUserForm.endpointMissing") : errorCode === "unauthorized" ? t("admin.manualUserForm.unauthorized") : t("admin.manualUserForm.requestFailed");
  ...
  return <section className="admin-panel admin-list-panel admin-manual-user-page">
    ...
    <form className="admin-editor admin-manual-user-form" onSubmit={handleSubmit}>
      ...
      <section className="admin-manual-section"><div className="admin-manual-section-heading"><span className="admin-manual-section-icon"><KeyRound size={17} /></span><div><p className="eyebrow">{t("admin.manualUserForm.credentialsEyebrow")}</p><h3>{t("admin.manualUserForm.credentialsTitle")}</h3><p>{t("admin.manualUserForm.credentialsCopy")}</p></div></div><div className="admin-access-options" role="radiogroup" aria-label={t("admin.manualUserForm.accessMethod")}><label className={draft.accessMode === "email-invite" ? "selected" : ""}><input type="radio" name="accessMode" value="email-invite" checked={draft.accessMode === "email-invite"} onChange={() => update("accessMode", "email-invite" as ManualAccessMode)} /><Send size={17} /><span><strong>{t("admin.manualUserForm.emailInvite")}</strong><small>{t("admin.manualUserForm.emailInviteCopy")}</small></span></label><label className={draft.accessMode === "temporary-password" ? "selected" : ""}><input type="radio" name="accessMode" value="temporary-password" checked={draft.accessMode === "temporary-password"} onChange={() => update("accessMode", "temporary-password" as ManualAccessMode)} /><KeyRound size={17} /><span><strong>{t("admin.manualUserForm.temporaryPassword")}</strong><small>{t("admin.manualUserForm.temporaryPasswordCopy")}</small></span></label></div>{draft.accessMode === "temporary-password" && <label className="admin-password-field"><span>{t("admin.manualUserForm.temporaryPasswordLabel")}</span><span className="admin-password-input"><input value={draft.temporaryPassword} onChange={(event) => update("temporaryPassword", event.target.value)} type={showPassword ? "text" : "password"} autoComplete="new-password" minLength={8} required aria-describedby="manual-password-requirements" /><button type="button" onClick={() => setShowPassword((current) => !current)} aria-label={t(showPassword ? "hidePassword" : "showPassword")} title={t(showPassword ? "hidePassword" : "showPassword")}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></span><small id="manual-password-requirements" className={`form-help password-requirement${draft.temporaryPassword && !isStrongPassword(draft.temporaryPassword) ? " invalid" : ""}`}>{t("passwordRequirements")}</small></label>}<label className="admin-checkbox-field"><span>{t("admin.manualUserForm.welcomeEmail")}</span><span className="admin-checkbox-row"><input type="checkbox" checked={draft.sendWelcomeEmail} onChange={(event) => update("sendWelcomeEmail", event.target.checked)} /><span>{t("admin.manualUserForm.welcomeEmailLabel")}</span></span></label></section>
